<!-- TenantUserListPage.vue - 租户用户管理页面
     职责：纯 UI 渲染与用户交互，业务逻辑由 lib/tenant-user.ts 提供 -->
<script setup lang="ts">
import { onMounted, ref } from 'vue'
import { useTenantUserList, ROLE_LABEL, formatDate, formatDateTime } from '@/lib/tenant-user'
...
import { Plus, Ban, CheckCircle, KeyRound, Check, Pencil, Trash2, MoreHorizontal } from 'lucide-vue-next'
...
const {
  ...
  loadUsers, createUser, openEditUser, updateUser, deleteUser, toggleUserStatus, openResetPassword, resetPassword,
} = useTenantUserList()
  ...
  <div class="space-y-6 text-base">
    <div class="flex items-center justify-end">
      <Tooltip>
        ...
        <TooltipContent>邀请管理员</TooltipContent>
      </Tooltip>
    </div>
    ...
    <div class="max-h-[420px] overflow-auto rounded-xl border">
      ...
      <p v-if="!loading && users.length === 0" class="py-6 text-center text-base text-muted-foreground">暂无用户，点击右上角邀请</p>
    </div>
    ...
    <Dialog v-model:open="showCreate">
      <DialogContent class="rounded-3xl">
        <DialogHeader>
          ...
          <DialogDescription>填写用户信息以邀请新的管理员加入该租户。</DialogDescription>
        </DialogHeader>
        <form class="flex flex-col gap-3" @submit.prevent="createUser">
          <div class="flex flex-col gap-1"><Label>用户名</Label><Input v-model="form.username" placeholder="用户名" /></div>
          <div class="flex flex-col gap-1"><Label>密码</Label><Input v-model="form.password" type="password"
              placeholder="至少8位" /></div>
          <div class="flex flex-col gap-1"><Label>姓名</Label><Input v-model="form.name" placeholder="显示名称" /></div>
          <div class="flex flex-col gap-1"><Label>邮箱（选填）</Label><Input v-model="form.email"
    ...
    <Dialog v-model:open="showEdit">
      <DialogContent class="rounded-3xl">
        <DialogHeader>
          ...
          <DialogDescription>更新用户姓名和邮箱信息。</DialogDescription>
        </DialogHeader>
        <form class="flex flex-col gap-3" @submit.prevent="updateUser">
          <div class="flex flex-col gap-1"><Label>姓名</Label><Input v-model="editForm.name" placeholder="显示名称" /></div>
          <div class="flex flex-col gap-1"><Label>邮箱（选填）</Label><Input v-model="editForm.email"
              placeholder="email@example.com" /></div>
          <DialogFooter>
            <Tooltip>
              <TooltipTrigger as-child>
    ...
    <Dialog v-model:open="showResetPwd">
      <DialogContent class="rounded-3xl">
        <DialogHeader>
          ...
          <DialogDescription>为指定用户设置新的登录密码。</DialogDescription>
        </DialogHeader>
